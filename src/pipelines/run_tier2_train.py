"""CLI for training the Tier 2 ACGD CNN-GRU-DSAM forecaster."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the Tier 2 CNN-GRU-DSAM NCI forecaster.")
    parser.add_argument("--config", type=Path, default=Path("configs/tier2_nci.yaml"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        from src.training.trainer_tier2 import train_tier2_from_windows
    ...
    configured_name = args.run_name or config.get("experiment", {}).get("run_name", "tier2_acgd")
    ...
    result = train_tier2_from_windows(config=config, windows_path=windows_path, run_name=run_name)
    print(f"Tier 2 training complete. best_epoch={result.best_epoch}, best_val_loss={result.best_val_loss:.6f}")
